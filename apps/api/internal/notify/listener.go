    ...
    go func() {
        ...
        for {
            ...
            if err != nil {
                ...
                select { case <-time.After(backoff): if backoff < 10*time.Second { backoff *= 2 }; continue; case <-ctx.Done(): return }
            }
            ...
            for {
                ...
                hub.Publish(stream.RunEvent{RunID: p.RunID, ThreadID: p.ThreadID, MessageID: p.MessageID})
            }
