...
type RunHandler struct{ U *usecase.Usecase; Hub *stream.Hub }
...
func (h *RunHandler) Stream(c echo.Context) error {
    ...
    if !ok { return c.String(http.StatusInternalServerError, "streaming unsupported") }

    writeEvent := func(event, data string) {
        if event != "" { _, _ = res.Write([]byte("event: "+event+"\n")) }
        if data != "" { _, _ = res.Write([]byte("data: "+data+"\n")) }
        _, _ = res.Write([]byte("\n"))
        flusher.Flush()
    }
